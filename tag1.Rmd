...
Dein R-Kurs beginnt gerade, aber Deine kleine Schwester nervt schon seit Stunden, dass Du ihre Mathe-Hausaufgaben überprüfen sollst bevor sie diese an den Lehrer schickt. "Wenn R so toll ist, dann kannst Du das doch bestimmt damit machen", sagt sie. Das stimmt, denn R ist turing-vollständig.
...
Das klappt. Also löst Du auch die restlichen Aufgaben. ACHTUNG: Kopieren und einfügen funktioniert hier nicht, Du musst die Zahlen eintippen!
...
Achtung: Hier benutzen wir `pi`, obwohl wir es nicht gesetzt haben; pi ist bereits von Haus in R vorhanden.
...
grade_this({
  ...
  # all objects created by student
  objs <- ls(env)
  
  # extract scalar numeric values
  values <- unlist(
  ...
  if (!(16 %in% values)) {
    fail("Es fehlt der Radius 16 cm.")
  }
  
  if (!(1.5 %in% values)) {
    fail("Es fehlt die Höhe 1.5 cm.")
  }
  ...
  pass_if(
    .result == target,
    "Richtig! Das Volumen wurde korrekt berechnet."
  )

  fail("Die Volumenberechnung stimmt noch nicht.")
...
R rechnet bei Vektoren elementweise. Ein Skalar wird dabei auf alle Elemente angewendet. Das ist ziemlich clever und geht in vielen anderen Programmiersprachen nicht!
...
Es gibt noch andere Datentypen, die wir im Laufe des Kurses benutzen werden. Datentypen in R und Skalenniveaus aus der Methodik sind nicht dasselbe, haengen aber oft zusammen. Numerische Variablen sind zum Beispiel haeufig intervall- oder verhaeltnisskaliert, kategoriale Variablen oft nominal oder ordinal. Der Preis, Durchmesser und der Preis pro Kubikzentimeter sind alle verhältnisskaliert, der Name der Pizza ist hingegen nominalskaliert. Den Datentyp bekommt man durch den Befehl `str` (structure, also Struktur) heraus:
...
Du kannst natürlich auch eine andere Variable als `pizzen` für die Zuweisung
benutzen. Eine Sache, die Dir hier auffallen sollte, ist, dass eine neue
Variable `X` entstanden ist. Diese enthält die Zeilennummern des ursprünglichen
Dataframes.

Der Grund ist, dass `write.csv()` standardmäßig die Zeilennamen speichert.
Beim Einlesen mit `read.csv()` werden diese dann als normale Variable
interpretiert.

Wenn wir das vermeiden möchten, können wir das Schreiben der Zeilennamen
bereits beim Speichern ausschalten:

```{r, echo = TRUE}
write.csv(d, file = "data/pizzen.csv", row.names = FALSE)
```

Dann entsteht beim Einlesen keine zusätzliche Variable:
...
Wow, jetzt kannst Du sogar alte SPSS-Daten in R einlesen und SPSS wegschmeißen. Hier nochmal eine kleine Übersicht über das Einlesen und Schreiben von Daten:

| Funktion        | Format                    | Wichtige Parameter                |
| --------------- | ------------------------- | --------------------------------- |
| `write.csv()`   | CSV (Komma getrennt)      | `file`, `row.names`               |
| `read.csv()`    | CSV (Komma getrennt)      | `file`, `header`                  |
| `write.csv2()`  | CSV2 (Semikolon getrennt) | `file`, `row.names`               |
| `read.csv2()`   | CSV2 (Semikolon getrennt) | `file`, `header`                  |
| `write.table()` | Text / Tabelle            | `file`, `sep`, `dec`, `row.names` |
| `read.table()`  | Text / Tabelle            | `file`, `header`, `sep`, `dec`    |
| `saveRDS()`     | RDS (R-Format)            | `file`                            |
| `readRDS()`     | RDS (R-Format)            | `file`                            |
| `read.spss()`   | SPSS (.sav)               | `file`, `to.data.frame`           |

Und die Parameter:

| Parameter       | Bedeutung                                                                |
| --------------- | ------------------------------------------------------------------------ |
| `file`          | Dateiname oder Pfad zur Datei                                            |
| `header`        | Gibt an, ob die erste Zeile Variablennamen enthält (`TRUE` oder `FALSE`) |
| `sep`           | Trennzeichen zwischen den Spalten (z. B. `","`, `";"`, `":"`)            |
| `dec`           | Dezimaltrennzeichen (`"."` oder `","`)                                   |
| `row.names`     | Gibt an, ob Zeilennummern gespeichert werden                             |
| `to.data.frame` | SPSS-Daten als Dataframe einlesen                                        |

Gar nicht übel, wie sich Deine Skills entwickeln:
