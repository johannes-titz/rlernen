...
Der Output der Analyse erfolgt am einfachsten über `summary`:

```{r}
summary(model1)
```

Koeffizienten, R², F-Statistik, Signifikanz, Standardfehler, Residualstandardfehler, usw. Das ist zwar alles da, aber nicht so schön aufbereitet.

Wir können uns hier auch eines schönen Pakets von Daniel Lüdecke bedienen:
...
Der Output kann über die Parameter von `tab_model` auch noch erweitert werden, schau mal in die Hilfe rein: `?tab_model`. Ein Nachteil ist allerdings, dass der Standardschätzfehler nicht angezeigt werden kann. Im summary-Output von oben ist er unter "Residual standard error" zu finden (1.028 in unserem Beispiel).

Nun zur Interpretation des Regressions-Outputs:
...
Eine leichte nicht-Linearität ist zu erkennen. Man sollte diese auch im Residual-Plot einer einfachen Regression sehen, den wir ganz einfach über plot bekommen:
...
Hier sehen wir die Residuen auf der Y-Achse und die vorhergesagten Werte auf der X-Achse. Es sollte kein Muster erkennbar sein, sondern die Punkte sollten zufällig verteilt sein -- Du erinnerst dich bestimmt daran noch aus der Vorlesung. Und dies ist hier eben nicht der Fall, wir sehen, dass niedrige und hohe Werte systematisch positive Residuen erzeugen. Dies ist ein Hinweis auf nicht-Linearität.

Wir können nun laut Potenzleiter entweder $x$ oder $y$ mit einem Exponenten der kleiner 1 ist versehen um die Krümmung auszugleichen. Üblicherweise transformiert man eher $x$, da die Interpretation für ein transformiertes $y$ schwierig ist. Aber wir müssen nicht unbedingt transformieren, sondern können einfach die gewünschte Beziehung mit `lm` spezifizieren. Vergleichen wir mal einen linearen Zusammenhang mit einem Exponenten von 0.5.
...
Ein letzter, aber wichtiger Hinweis: obwohl wir uns hier hauptsächlich auf Transformationen über Potenzen konzentriert haben, kann man natürlich auch ganz andere Transformationen vornehmen. Wenn man z. B. denkt, dass der Zusammenhang sinus-förmig ist, spricht nichts dagegen einfach sin(x) als Transformation zu verwenden.
