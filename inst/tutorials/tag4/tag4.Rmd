...
description: >
  Inferenzstatistik between und within: klassischer t-Test und klassische ANOVA, Kontrast-Analyse als Alternative zur ANOVA
tutorial:
  version: 2.0
output:
  learnr::tutorial:
...
Wir fangen mit $t$-Tests an, behandeln anschließend die ANOVA und am Ende die Kontrast-Analyse. Wir nutzen wieder die Vorlesungsdaten:
...
Wir übergeben den Vektor, den wir testen wollen (vorher filtern und selektieren!) und geben die Konstante über den Parameter `mu` an. Wir sehen, dass in unserer Stichprobe die Körpergröße bei 168.7cm liegt, gleichzeitig ist dieses Ergebnis nicht signifikant verschieden von 168cm. Die Wahrscheinlichkeit dieses Ergebnisses oder eines extremen, sofern die 168cm in der Population korrekt ist, liegt bei 0.3977. Also durchaus wahrscheinlich.

Prüfe nun nach ob das gleiche für SeKo-Studentinnen gilt.

```{r konstante, exercise=TRUE, exercise.reveal_solution = FALSE}
...
```{r echo = TRUE}
t.test(data$hunde_m, data$katzen_m, paired = TRUE)
```
...
Rechne nun einen Test für den Unterschied zwischen den Deutsch- und Mathepunkten (diese sind natürlich abhängig). Vergleiche diesen anschließend mit der Version für unabhängige Stichproben (gleiche Varianzen).
...
t.test(data$deutsch, data$mathe, var.equal = TRUE)
...
Hier sieht man ganz konkret, dass $r_\mathrm{effect size}$ die Korrelation zwischen Lambdas und der abhängigen Variable ist (auf Individuum-Ebene).

Um die Kontrast-Analyse normal zu berechnen können wir das Paket ``cofad`` benutzen:
...
Rechne die Kontrast-Analyse aus der Vorlesung auf Folie 21 mit `cofad` nach:
