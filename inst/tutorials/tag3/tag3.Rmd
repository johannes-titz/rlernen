...
Probier es nun selbst. Berechne mit `colMeans` die Mittelwerte der Variablen **alter**, **mathe**, **deutsch**, **abi** und **lebenszufriedenheit**.
...
Und nun mit `rowMeans`: Berechne für jede Person den Mittelwert über genau diese fünf Variablen.
...
Das ganze geht auch für **Summen** über `rowSums` und `colSums`. Aber was machen wir, wenn wir z. B. die **Standardabweichung** (`sd`) oder die **Varianz** (`var`) über mehrere Variablen berechnen wollen? Dafür gibt es keine speziellen Funktionen wie `rowSd` oder `colSd`.
