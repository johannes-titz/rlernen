...
description: >
  Tabellen, Lage- und Streuungsmaße, einfache Abbildungen mit base R, fehlende Werte, eigene Funktionen schreiben
tutorial:
  version: 3.0
output:
  learnr::tutorial:
...
librarian::shelf(psych, meme, rlernen)
tutorial_setup()
gradethis::gradethis_setup()
modus <- function(x) {
  tab <- table(x)
  max_frequency <- max(tab)
  modus <- names(tab[tab == max_frequency])
  return(modus)
}
...
```{r tab3, echo = TRUE}
round(proportions(table(data$mathe, data$geschlecht)), 2)
```

Ein kleiner Check: Beträgt die Summe dieser Tabelle 1 (entspricht 100%)?

```{r tab3b, echo = TRUE}
sum(proportions(table(data$mathe, data$geschlecht)))
```

`proportions` selbst nimmt als Argument eine normale Tabelle, die wir vorher über `table` erzeugen. Das wirkt bei Rohdaten zunächst etwas umständlich, ist aber ganz praktisch, wenn wir von irgendwoher eine fertige Tabelle bekommen und dort die relativen Anteile wissen wollen:
...
`margin` bedeutet Rand und gibt an, entlang welcher Dimension normiert wird (Zeile oder Spalte). Bei einer ANOVA-Tabelle machen wir so etwas ähnliches, benutzen jedoch Mittelwerte. Die 2 bei `margin` bedeutet Spalte (1 wäre die Zeile).
...
Gib mir den relativen Anteil der Punkte in Deutsch nach dem Geschlecht aus. Die Summe über das Geschlecht soll in diesem Fall 1 (entspricht 100%) ergeben. Zum Beispiel: für 6 Punkte, männlich 0.25, weiblich 0.75.
...
var(data$alter)
range(data$alter)
mean(abs(data$alter - mean(data$alter))) # aufpassen!
...
Nun haben wir die wichtigsten Lage- u. Streuungsmaße behandelt. Bevor wir weitermachen, gibt es noch einen kleinen Exkurs zu Hilfsfunktionen, die bei der Berechnung von deskriptiven Statistiken hilfreich sind.

## Hilfsfunktionen

Wenn Du den Mittelwert nicht nur für **eine** Spalte, sondern gleich für **mehrere Spalten** berechnen willst, hilft Dir die Funktion `colMeans`:

```{r lage2, echo = TRUE}
colMeans(data[, c("alter", "mathe", "deutsch", "abi", "lebenszufriedenheit")])
```

Hier wird also für jede ausgewählte Variable der Mittelwert berechnet.

Es geht aber auch andersherum. Mit `rowMeans` kannst Du für **jede Person** den Mittelwert über mehrere Variablen berechnen:

```{r lage3, echo = TRUE}
rowMeans(data[, c("alter", "mathe", "deutsch", "abi", "lebenszufriedenheit")])
```

Hier wird für jede Zeile ein Mittelwert gebildet.


Das ganze geht auch für **Summen** über `rowSums` und `colSums`. Probier es selbst. Berechne mit `colSums` die Summen der Variablen **alter**, **mathe**, **deutsch**, **abi** und **lebenszufriedenheit**.

```{r lage5, exercise=TRUE}

```

```{r lage5-solution}
colSums(data[, c("alter", "mathe", "deutsch", "abi", "lebenszufriedenheit")])
```

Und nun mit `rowSums`: Berechne für jede Person den Mittelwert über genau diese fünf Variablen.

```{r lage6, exercise=TRUE}

```

```{r lage6-solution}
rowSums(data[, c("alter", "mathe", "deutsch", "abi", "lebenszufriedenheit")])
```

Was machen wir, wenn wir z. B. die **Standardabweichung** (`sd`) oder die **Varianz** (`var`) über mehrere Variablen berechnen wollen? Dafür gibt es keine speziellen Funktionen wie `rowSd` oder `colSd`.

In solchen Fällen kann man die Funktion `apply` benutzen.

```{r echo=TRUE}
apply(data[, c("alter", "mathe", "deutsch")], 2, sd)
```

Das sieht erst einmal etwas kryptisch aus. Die Funktion `apply` hat aber eine einfache Logik:

- Das **erste Argument** ist der Datensatz oder die Matrix, auf die wir die Funktion anwenden wollen.
- Das **zweite Argument** bestimmt, ob wir über **Zeilen** oder **Spalten** gehen.
- Das **dritte Argument** ist die Funktion, die angewendet werden soll.

Die Zahl **2** bedeutet hier: *wende die Funktion auf die Spalten an*. In diesem Beispiel wird also für jede Variable (`alter`, `mathe`, `deutsch`) die Standardabweichung berechnet.

Für **Zeilen** funktioniert das genauso:

```{r, echo = T}
apply(data[, c("alter", "mathe", "deutsch")], 1, sd)
```

Hier bedeutet die **1**, dass die Funktion auf **Zeilen** angewendet wird. Es wird also für jede Person die Standardabweichung über die drei Variablen berechnet.

Du kannst `sd` natürlich auch durch eine andere Funktion ersetzen, zum Beispiel `mean`, `var` oder `sum`.

Probier Du es jetzt mal selbst.

Berechne mit `apply` die **Varianz der Variablen** `alter`, `mathe` und `deutsch`.

```{r apply1, exercise=TRUE}

```

```{r apply1-solution}
apply(data[, c("alter", "mathe", "deutsch")], 2, var)
```

Berechne nun mit `apply` den **Median über die Variablen** `alter`, `mathe` und `deutsch` für **jede Person**.

```{r apply2, exercise=TRUE}

```

```{r apply2-solution}
apply(data[, c("alter", "mathe", "deutsch")], 1, median)
```

## Fehlende Werte

In echten Datensätzen fehlen oft einzelne Angaben. In `R` werden fehlende Werte als `NA` gespeichert.  
Für Beispiele zu fehlenden Werten verwenden wir hier den Datensatz `bfi` aus dem Paket `psych`, da dieser tatsächlich fehlende Werte enthält.
...
Schauen wir uns das am Beispiel der Variable `A1` an:

```{r na1, echo=TRUE}
table(bfi$A1, useNA = "always")
sum(is.na(bfi$A1))
```

Mit `useNA = "always"` wird in einer Tabelle auch angezeigt, wie viele fehlende Werte es gibt. Die Funktion `is.na()` prüft für jeden Wert, ob er fehlt, und mit `sum()` zählen wir dann die fehlenden Werte.

Viele Funktionen geben `NA` zurück, sobald fehlende Werte vorkommen:
...
Mit `na.rm = TRUE` werden fehlende Werte vor der Berechnung entfernt:

```{r na3, echo=TRUE}
mean(bfi$A1, na.rm = TRUE)
sd(bfi$A1, na.rm = TRUE)
median(bfi$A1, na.rm = TRUE)
```

Auch bei Korrelationen muss man aufpassen. Hier kann man vollständige Fälle verwenden:

```{r na4, echo=TRUE}
cor(bfi$A1, bfi$A2, use = "complete.obs")
```

Das bedeutet: Es werden nur die Personen berücksichtigt, für die auf beiden Variablen tatsächlich Werte vorliegen.

### Übungsbeispiele

Zähle die Anzahl fehlender Werte in der Variable `education`. Speichere anschließend den Mittelwert, die Standardabweichung und eine Tabelle der Werte so, dass fehlende Werte angemessen berücksichtigt werden.

```{r na6, exercise=TRUE, exercise.reveal_solution = FALSE}
n_missing <- ___
mean_no_na <- ___
sd_no_na <- ___
tab_with_na <- ___
...
```{r na6-hint-1}
# Schau Dir die Beispiele oben nochmal an.
# Du brauchst is.na(), na.rm = TRUE und useNA = "always".
```

```{r na6-solution}
n_missing <- sum(is.na(bfi$education))
mean_no_na <- mean(bfi$education, na.rm = TRUE)
sd_no_na <- sd(bfi$education, na.rm = TRUE)
tab_with_na <- table(bfi$education, useNA = "always")
```
...
Berechne die Korrelation zwischen `A1` und `A2`, sodass nur vollständige Fälle verwendet werden.
...
```{r na7-hint-1}
# Die Funktion cor() hat ein Argument namens use.
```
...
### Manuelle Entfernung von fehlenden Werten

Bisher haben wir Funktionen kennengelernt, die das Argument `na.rm = TRUE` besitzen. Damit können fehlende Werte automatisch vor der Berechnung entfernt werden.

Es gibt jedoch auch Funktionen in `R`, die **kein solches Argument haben**. In diesen Fällen müssen wir fehlende Werte **manuell entfernen**.

Da wir bisher nicht so viele Funktionen kennen, machen wir einfach ein konstruiertes Beispiel. Sagen wir mal, dass wir nicht wissen, dass `median` das Argument `na.rm` besitzt. Die normal Eingabe führt mit NA-Werten natürlich zu keinem sinnvollen Ergebnis:

```{r na8, echo=TRUE, error=TRUE}
median(bfi$A1)
```

Um das Problem zu lösen, entfernen wir die fehlenden Werte **vor der Berechnung** über `na.omit`.

```{r na9, echo=TRUE}
median(na.omit(bfi$A1))
```

Das Ergebnis ist also ein Vektor **ohne fehlende Werte**, auf den wir dann `median()` anwenden können.

Berechne die Varianz der Variable `A2`, ohne dass fehlende Werte das Ergebnis beeinflussen. Nutze `na.omit`!

```{r na11, exercise=TRUE}

```

```{r na11-hint-1}
# Entferne fehlende Werte vor der Berechnung mit na.omit().
```
...
Dieses Vorgehen ist immer dann notwendig, wenn eine Funktion kein Argument wie `na.rm` besitzt. Die meisten Basisfunktionen in `R` haben jedoch ein solches Argument, sodass wir `na.omit` nicht so häufig brauchen werden.

`na.omit` funktioniert auch bei einem data.frame und entfernt alle Zeilen, die mindestens einen fehlenden Wert enthalten:

```{r echo=TRUE}
nrow(bfi)
bfi_without_na <- na.omit(bfi)
nrow(bfi_without_na)
```

Hier sehen wir, dass sich die Anzahl der Zeilen (`nrow`) deutlich reduziert hat, nachdem wir `na.omit` eingesetzt haben. Falls zu viele Werte fehlen, ist dieses Vorgehen nicht zu empfehlen und man sollte stattdessen Imputations-Verfahren anwenden (also fehlende Werte durch plausible Werte ersetzen). Das ist jedoch ein sehr komplexes Thema, das wir hier nicht behandeln werden.

## Exkurs: Eigene Funktionen schreiben

Tatsächlich fehlt uns noch ein Lagemaß, nämlich der Modus. Wie Du schon aus Tag 1 weißt, gibt es keine Funktion in `R` für den Modus (zumindest ohne Zusatzpakete). Wir werden uns jetzt eine eigene Funktion dafür schreiben. Das ist ein relativ fortgeschrittenes Thema, aber wir sind ja auch schon bei Tag 3. Konzentrier Dich gut, dann hast Du bald die Möglichkeit `R`  so zu erweitern, wie Du willst.
...
Hier benutzen wir die Tilde `~`. Das ist ein spezielles Symbol in `R`, wir werden es uns später nochmal genauer anschauen. Merk Dir einfach, dass die Syntax der Idee folgt `AV ~ UV1 + UV2 + ... + UVn` (links steht die AV, rechts die UVs, getrennt durch die Tilde) -- bitte frag mich jetzt nicht wo die Tilde auf der Tastatur ist.
...
Viele behaupten ja, dass man entweder gut in Deutsch oder Mathe ist. Dies ist offensichtlich nicht der Fall, denn die Korrelation zwischen beiden ist sehr hoch (unter Annahmen der KTT könnte die wahre Korrelation noch höher sein). Aus Perspektive der meisten Intelligenztheorien ist dies ein zu erwartendes Resultat.
...
question("Welche Gruppe hat den niedrigsten Median in Lebenszufriedenheit?",
  ...
  answer("vegetarische Meditierer"),
...
q_n(0, "Wie groß ist die aufgeklärte *Varianz* in der Lebenszufriedenheit durch die Abiturnote? (in Prozent auf eine ganze Zahl gerundet)")
```
...
Programmiere eine eigene Funktion, die eine Variable $z$-standardisiert. Wende Sie anschließend auf die Mathe- und Deutschnote an. Multipliziere schließlich alle $z$-Werte der Mathe- und Deutschpunkte, summiere alle Wert auf und teile durch die Stichprobengröße (`nrow(data)`).
...
sum(zstandardisiere(data$mathe) * zstandardisiere(data$deutsch)) / nrow(data)
```
...
Die Funktion var() berechnet die Stichprobenvarianz mit Nenner $n - 1$. Diese wird verwendet, um die Populationsvarianz unverzerrt zu schätzen. Schreib eine Funktion, die die Varianz einer Stichprobe berechnet (im Nenner steht $n$). Vergleiche anschließend die absolute Abweichung der beiden Funktionen für die Variable Alter.
...
Nun Du. Wenn Du nicht weiter weißt, dann mache eine Websuche und finde die notwendigen Parameter.
